...
from .mmutils import is_online, emojis, clean_url, url2file, check_yolov5u_filename
...
def safe_download(url,
    ...
    if unzip and f.exists() and f.suffix in ('', '.zip', '.tar', '.gz'):
        ...
        print(f'Unzipping {f} to {unzip_dir}...')
        if is_zipfile(f):
...
def attempt_download_asset(file, repo='ultralytics/assets', release='v0.0.0'):
    ...
    def github_assets(repository, version='latest'):
        ...
        print(f'https://api.github.com/repos/{repository}/releases/{version}')
