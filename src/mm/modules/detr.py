...
CFG_DICT = {
    "m": "detr_r50_8xb2-150e_coco.py",
}

CFG_B_DICT = {
    "detr_r50_8xb2-150e_coco.py": "m",
}

def build_detr(current_directory,
               model_name, model_size,
               model_weight_path, input_array=True):
    assert model_size in CFG_DICT.keys(), f'model_size must be {CFG_DICT.keys()}'
    cfg_path = os.path.join(current_directory, "configs", "detr", CFG_DICT[model_size])
