...
class Results(SimpleClass):
    ...
    def pandas(self):
        ...
        print("WARNING ⚠️ 'Results.pandas' method is not yet implemented.")
    ...
    def plot(self):
        raw_img = self.orig_img.copy()
        for bbox in self.boxes:
            xyxy = [int(n) for n in bbox.xyxy[0]]
            class_id = bbox.cls
            class_name = self.names[int(class_id)]
            conf = bbox.conf.item()
            cv2.rectangle(raw_img, (xyxy[0], xyxy[1]), (xyxy[2], xyxy[3]), (0, 255, 0), 2)
            cv2.putText(raw_img, class_name, (xyxy[0], xyxy[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        return raw_img    
...
class Boxes(BaseTensor):
    ...
    def boxes(self):
        ...
        print("WARNING ⚠️ 'Boxes.boxes' is deprecated. Use 'Boxes.data' instead.")
        return self.data
...
class Masks(BaseTensor):
    ...
    def segments(self):
        ...
        print("WARNING ⚠️ 'Masks.segments' is deprecated. Use 'Masks.xyn' for segments (normalized) and "
    ...
    def masks(self):
        ...
        print("WARNING ⚠️ 'Masks.masks' is deprecated. Use 'Masks.data' instead.")
        return self.data

    def pandas(self):
        """Convert the object to a pandas DataFrame (not yet implemented)."""
        print("WARNING ⚠️ 'Masks.pandas' method is not yet implemented.")