...
from .mmmetrics import bbox_ioa
from .mmutils import check_version, colorstr
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if self.auto:  # minimum rectangle
            ...
            # print("auto", dw, dh, isinstance(dw, torch.Tensor), isinstance(dh, np.ndarray), self.stride)
        elif self.scaleFill:  # stretch
            dw, dh = 0.0, 0.0
            new_unpad = (new_shape[1], new_shape[0])
            ratio = new_shape[1] / shape[1], new_shape[0] / shape[0]  # width, height ratios
            # print("scaleFill", dw, dh)
        # print(dw, dh)
...
def v8_transforms(dataset, imgsz, hyp):
    ...
    if dataset.use_keypoints and flip_idx is None and hyp.fliplr > 0.0:
        hyp.fliplr = 0.0
        print("WARNING ⚠️ No `flip_idx` provided while training keypoints, setting augmentation 'fliplr=0.0'")
    return Compose([
        pre_transform,
